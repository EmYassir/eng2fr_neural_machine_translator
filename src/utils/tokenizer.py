...
def main():
    ...
    for current_file in args.input:
        ...
        logger.info(f'done ({done} / {len(args.input)}) - skipped {empty_skipped} lines (over {tot_lines} '
                    f'- i.e., {(empty_skipped / tot_lines) * 100:3.2f}%) because empty,'
                    f' skipped {regex_skipped} lines (over {tot_lines} '
                    f'- i.e., {regex_skipped / tot_lines * 100:3.2f}%) because of regex')
